package Phase_Two.Day10;

import java.io.*;
import java.util.*;

public class RollNO {
    static BufferedReader br = new BufferedReader(new InputStreamReader(System.in));
    static StringTokenizer st;
    static PrintWriter out = new PrintWriter(new BufferedWriter(new OutputStreamWriter(System.out)));
    static StringBuilder sb = new StringBuilder();

    // Constants
    static final long MOD  = 1_000_000_007L;
    static final long MOD2 = 998_244_353L;
    static final int  INF  = (int) 1e9;
    static final long LINF = (long) 1e18;
    static final double EPS = 1e-9;

    // Local testing — change to false before submitting!
    static final boolean LOCAL = true;

    static String next() throws IOException {
        while (st == null || !st.hasMoreTokens())
            st = new StringTokenizer(br.readLine());
        return st.nextToken();
    }
    static int nextInt() throws IOException { return Integer.parseInt(next()); }
    static long nextLong() throws IOException { return Long.parseLong(next()); }
    static double nextDouble() throws IOException { return Double.parseDouble(next()); }
    static char nextChar() throws IOException { return next().charAt(0); }
    static String nextLine() throws IOException {
        st = null;
        return br.readLine();
    }

    public static void main(String[] args) throws Exception {
        if (LOCAL) {
            br = new BufferedReader(new FileReader("../input.txt"));
        }
        int t = 1;       // change to nextInt() for multiple test cases
        while (t-- > 0) solve();
        out.print(sb);
        out.flush();
    }

    static void solve() throws Exception {
        // int n = nextInt();
        // TreeMap<Integer,String> map = new TreeMap<>();
        // while(n-->0){
        //     String s = next();
        //     int roll = nextInt();
        //     map.put(roll, s);
        // }
        // for(Map.Entry<Integer, String> mp :map.entrySet()){
        //     sb.append(mp.getValue()).append(" ").append(mp.getKey()).append("\n");
        // }
        int n = nextInt();
        Roll[] rol = new Roll[n];
        for (int i = 0; i < n; i++){
            String s = next();
            int roll = nextInt();
            rol[i] = new Roll(s, roll);
        }
        Arrays.sort(rol,new Comparator<Roll>() {
            public int compare(Roll a, Roll b){
                return Integer.compare(a.roll, b.roll);
            }
        });
        for (int i = 0; i < n; i++){
            sb.append(rol[i].s).append(" ").append(rol[i].roll).append("/n");
        }
    }
    static class Roll{
        String s;
        int roll;
        Roll(String str, int rolly){
            this.s = str;
            this.roll = rolly;
        }
    }
}