...
public class CarAndPetrol2 {
    ...
    static void solve() throws Exception {
        ...
        for(int i=0; i<n-1; i++){
            set.add(new int[]{p[i],c[i]});
            int cover = r[i];
            while (!set.isEmpty()&&cover!=0) {
                int[] a =set.pollFirst();
                if(a[1]<=cover){
                    cover-=a[1];
                    dol += a[1]*a[0];
                    ...
                    a[1]-=cover;
                    cover =0;
                    set.add(new int[]{a[0],a[1]});
                }
                
            }
            if(cover>0){
                sb.append("ImPossible to cover the distance in this condition");
            return;}
        }
        sb.append(dol);
    }
